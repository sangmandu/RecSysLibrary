...
### The Current Systems
Architecture에서 소개한 시스템 구조는 소수의 연구자들이 사용한다. 이후의 파트에서는 Tapestry 시스템의 다양한 구성요소를 소개할 것임

### Database Manager
Tapestry는 doc, anno, filter query를 Sybase에서 제공하는 관계형 데이터베이스에 저장. 메세지에 관한 정보는 관계형 표에 저장됨. 단일표는 관계형 모델에 적합하지 않은데, 이는 단일표로 표현할 수 있는 특징이 없기 때문. 메세지의 특징들은 수신자나 뉴스그룹과 같이 집합형 값으로 이루어져 있는데 이러한 값은 . 모든 메세지에 공통으로 있는 정보들은 집합형 값이 아니기 때문에 메세지 당 하나의 공간이 있는 표에 저장됨. 메세지마다 다른 정보들은 보조 테이블에 저장됨.


글에 대해 전반적인 이해가 어려워 해설이 아닌 직역을 하고 있는 것 같아 작성을 멈춤.
